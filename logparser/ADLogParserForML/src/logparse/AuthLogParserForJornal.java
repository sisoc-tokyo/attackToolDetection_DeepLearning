 ...
 */
public class AuthLogParserForJornal {
	...
	private void outputResults(Map map, String outputFileName) {
		try {
			...
			pw.println("datetime,eventid,accountname,clientaddr,servicename,processname,objectname,sharedname");
		...
		for (Iterator it = kerlog.entrySet().iterator(); it.hasNext();) {
			...
			for (EventLogData ev : evS) {
				...
				if (eventID == EVENT_PRIV_OPE || eventID == EVENT_PROCESS
						|| eventID == EVENT_SHARE || eventID == EVENT_TGT || eventID == EVENT_ST) {
					try {
						if(sdf.parse(ev.getDate()).before(sdf.parse("2019/12/25 00:00:00"))) {
							continue;
						}
					...
					pw.println(ev.getDate()+ "," +ev.getEventID() + "," +accountName + ","
							+ ev.getClientAddress() + "," +ev.getServiceName()+ ","
							+ ev.getProcessName() + "," + ev.getObjectName() + ","
							+ ev.getSharedName());
				}
			}
		}
	...
	public void detectGolden(String inputDirname) {
		...
		outputResults(log, this.outputDirName + "/" + "log.csv");
	}
	...
	public static void main(String args[]) throws ParseException {
		AuthLogParserForJornal authLogParser = new AuthLogParserForJornal();
		String inputdirname = "";
		String commandFile = "";
		String adminlist = "";
		if (args.length < 3) {
